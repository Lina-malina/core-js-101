...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startParenthesis}${min}, ${max}${endParenthesis}`;
}
...
function isCreditCardNumber(ccn) {
  const arr = Array.from(String(ccn)).map((x) => Number(x));
...
function getDigitalRoot(num) {
  const arr = Array.from(String(num)).map((x) => Number(x));
  let sum = arr.reduce((cur, acc) => acc + cur);
  if (sum > 9) {
    sum = Array.from(String(sum)).map((x) => Number(x)).reduce((cur, acc) => acc + cur);
  }
  return sum;
}
